...
public protocol LightboxControllerTapDelegate: AnyObject {
    ...
    @MainActor
    func lightboxController(_ controller: LightboxController, didDoubleTap image: LightboxImage, at index: Int, hasZoomed: Bool)
    @MainActor
    func lightboxController(_ controller: LightboxController, didPinch image: LightboxImage, at index: Int, hasZoomed: Bool)
}
...
extension LightboxController: PageViewDelegate {
    ...
    func pageViewDidDoubleTap(_ pageView: PageView) {
        imageTapDelegate?.lightboxController(self, didDoubleTap: images[currentPage], at: currentPage, hasZoomed: pageView.hasZoomed)
    }
    
    func pageViewDidPinch(_ pageView: PageView) {
        let duration = pageView.hasZoomed ? 0.1 : 0.5
        toggleControls(pageView: pageView, visible: !pageView.hasZoomed, duration: duration, delay: 0.5)
        imageTapDelegate?.lightboxController(self, didPinch: images[currentPage], at: currentPage, hasZoomed: pageView.hasZoomed)
    }
}
